...
def uniqueify(x:Any, sort:bool=False)->List:
...
def series2cat(df:Any, *col_names):
...
def df_names_to_idx(names:IntsOrStrs, df:Any):
...
def index_row(a:Any, idxs:Collection[int])->Any:
    "Return the slice of `a` corresponding to `idxs`."
    if a is None: return a
    if isinstance(a, any):
        res = a.iloc[idxs]
        if isinstance(res,any): return res.copy()
        return res
    return a[idxs]
